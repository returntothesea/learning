...
# characters within the string into groups. (i.e. if there are M dashes, the string is split into M+1
...
# which could be shorter, but still must contain at least one character). To satisfy this requirement,
# we will reinsert dashes. Additionally, all the lower case letters in the string must be converted to upper case.
...
def format(string, num)
	string = string.gsub("-","").upcase.reverse
	arr = string.scan /.{1,#{num}}/
	string = arr.join("-").reverse
	return string
end
